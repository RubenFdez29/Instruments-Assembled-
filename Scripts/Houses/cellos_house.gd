...
#Función que se ejecuta al entrar en la casa del Cello desde su mundo.
# Reproduce su música en bucle.
func _ready():
	$Cellos_travel.play()
	if global.first_time == true:
		DialogueManager.show_example_dialogue_balloon(load("res://Dialogues/Voices.dialogue"), "cellos_house")
...
func _process(delta):
	...
	if Input.is_action_just_pressed("Pause"):
		$Cellos_travel.get_stream_paused()
